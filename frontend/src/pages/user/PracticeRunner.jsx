...
import { ArrowLeft, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Flag, GripVertical, Lightbulb, Mic, PenLine, RotateCcw, Volume2, XCircle } from 'lucide-react';
...
export default function PracticeRunner() {
  ...
  useEffect(() => {
    api.get(`/practice-exams/${id}`).then((r) => {
      setExam(r.data.data);
  ...
  useEffect(() => {
    if (!exam || submitted || timeLeftSeconds === null) return undefined;
  ...
  }, [answers, exam, submitted, timeLeftSeconds]);

  const result = useMemo(() => {
    if (!exam) return { correct: 0, total: 0, score: 0, answered: 0, scorable: 0, manual: 0 };
    const questions = exam.questions || [];
    const answered = questions.filter((q) => hasAnswer(q)).length;
    const manual = questions.filter((q) => !isAutoScorableQuestion(q)).length;
    const summary = questions.reduce((acc, q) => {
      const item = questionScore(q);
      acc.correct += item.correct;
      acc.total += item.total;
      if (item.total > 0) acc.scorable += 1;
      return acc;
    }, { correct: 0, total: 0, scorable: 0 });
    return {
      ...summary,
      answered,
      manual,
      score: summary.total ? Math.round((summary.correct * 100) / summary.total) : 0
    };
  }, [answers, exam]);

  if (!exam) return <div className="panel text-slate-500">Đang tải...</div>;
  ...
  function isAutoScorableQuestion(q) {
    return Boolean(q.correctAnswer?.trim()) && !isSpeakingPrompt(q) && !isWritingQuestion(q);
  }

  function questionScore(q, answerMap = answers) {
    if (!isAutoScorableQuestion(q)) return { correct: 0, total: 0 };
    const selected = answerMap[q.id] || '';
    const correctParts = answerParts(q.correctAnswer).filter(Boolean);
    if (correctParts.length > 1) {
      const selectedParts = answerParts(selected);
      return {
        correct: correctParts.filter((part, index) => normalizeAnswer(selectedParts[index]) === normalizeAnswer(part)).length,
        total: correctParts.length
      };
    }
    return {
      correct: normalizeAnswer(selected) === normalizeAnswer(q.correctAnswer) ? 1 : 0,
      total: 1
    };
  }

  function resultLevel(score) {
    if (score >= 85) return { label: 'Rất tốt', className: 'bg-green-50 text-green-700 border-green-200' };
    if (score >= 70) return { label: 'Đạt', className: 'bg-blue-50 text-blue-700 border-blue-200' };
    if (score >= 50) return { label: 'Cần luyện thêm', className: 'bg-amber-50 text-amber-700 border-amber-200' };
    return { label: 'Chưa đạt', className: 'bg-red-50 text-red-700 border-red-200' };
  }
  ...
  function renderScorePanel() {
    ...
    return (
      <section className={`rounded-md border p-5 shadow-sm ${level.className}`}>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-sm font-black uppercase">Kết quả luyện thi</div>
            <h2 className="mt-1 text-3xl font-black">{result.score}/100 - {level.label}</h2>
            <p className="mt-1 text-sm opacity-80">
              Đúng {result.correct}/{result.total} ý chấm tự động. Đã làm {result.answered}/{exam.questions?.length || 0} câu.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-sm">
            <div className="rounded-md bg-white/75 px-4 py-3">
              <div className="text-2xl font-black">{result.correct}</div>
              <div>Ý đúng</div>
            </div>
            <div className="rounded-md bg-white/75 px-4 py-3">
              <div className="text-2xl font-black">{Math.max(result.total - result.correct, 0)}</div>
              <div>Ý sai</div>
            </div>
            <div className="rounded-md bg-white/75 px-4 py-3">
              <div className="text-2xl font-black">{result.manual}</div>
              <div>Tự luận</div>
            </div>
          </div>
        </div>
        {result.manual > 0 && (
          <p className="mt-4 rounded-md bg-white/75 p-3 text-sm">
            Các câu Writing/Speaking hiện hiển thị đáp án mẫu để tự đối chiếu; điểm trên chỉ tính các câu có đáp án đúng cố định.
          </p>
        )}
      </section>
    );
  ...
  function isQuestionReviewed(q) {
    return submitted || checkedQuestions.includes(q.id);
  }
      ...
      <section className="space-y-5">
        ...
        <div className="question-action-bar md:left-[260px]">
          <div className="question-action-grid question-action-grid--three">
            ...
            <button type="button" className="btn btn-primary question-action-btn" onClick={submit}><CheckCircle2 size={16} />Chấm điểm</button>
          </div>
        </div>
        {speakingImageAnswer && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
  ...
  function renderSpeakingPracticeScreen(screen, activeIndex, total) {
    return (
      <section className="space-y-4">
        ...
        {screen.type === 'part4' && renderSpeakingPart4Card(screen.q, 3, true)}
  ...
  function renderMixedWritingQuestion(q, index) {
        ...
        <div className="space-y-4">
          ...
          {writingEmailPrompts(q).map((prompt, promptIndex) => (
            <div key={promptIndex} className="space-y-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <textarea id={`mixed-writing-${q.id}-${promptIndex}`} className="input min-h-36" value={writingChatValue(q)[promptIndex] || ''} disabled={submitted} onChange={(event) => setWritingChatAnswer(q, promptIndex, event.target.value)} />
              <div className="flex flex-wrap items-center justify-between gap-3">
                {renderWritingHintButton(q, writingPromptHintKey(q, promptIndex))}
                <div className="rounded-md bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">Word Count: {wordCount(writingChatValue(q)[promptIndex] || '')}</div>
              </div>
        ...
        <div className="space-y-4">
          {writingChatPrompts(q).map((prompt, promptIndex) => (
            <div key={promptIndex} className="space-y-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <textarea id={`mixed-writing-${q.id}-${promptIndex}`} className="input min-h-24" value={writingChatValue(q)[promptIndex] || ''} disabled={submitted} onChange={(event) => setWritingChatAnswer(q, promptIndex, event.target.value)} />
              <div className="flex flex-wrap items-center justify-between gap-3">
                {renderWritingHintButton(q, writingPromptHintKey(q, promptIndex))}
                <div className="rounded-md bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">Word Count: {wordCount(writingChatValue(q)[promptIndex] || '')}</div>
              </div>
    ...
    if (isWritingLongQuestion(q)) {
      return (
        <div className="space-y-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <textarea id={`mixed-writing-${q.id}`} className="input min-h-44" value={answers[q.id] || ''} disabled={submitted} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.value })} />
          <div className="flex flex-wrap items-center justify-between gap-3">
            {renderWritingHintButton(q)}
            <div className="rounded-md bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">Word Count: {wordCount(answers[q.id] || '')}</div>
          </div>
    ...
    return (
      <div className="space-y-3">
        ...
        <input id={`mixed-writing-${q.id}`} className="input" placeholder="Type your answer here" value={answers[q.id] || ''} disabled={submitted} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.value })} />
        {renderWritingHintButton(q)}
        {visibleAnswers[q.id] && renderWritingCompactHint(q)}
      </div>
    );
  ...
  function checkQuestion(q) {
    ...
    if (q.questionType === 'PARAGRAPH_ORDER' && !nextAnswers[q.id]) nextAnswers[q.id] = orderValue(q).join(',');
    if (q.questionType === 'MATCHING_DROPDOWN' && !nextAnswers[q.id]) nextAnswers[q.id] = q.optionF === 'GRAMMAR_TERM_MATCH' ? grammarTermMatchValue(q).join('|||') : matchingValue(q).join(',');
    if (q.optionF === 'GRAMMAR_SENTENCE_DROPDOWN' && !nextAnswers[q.id]) nextAnswers[q.id] = grammarSentenceDropdownValue(q).join('|||');
    if (q.questionType === 'OPINION_MATCH' && !nextAnswers[q.id]) nextAnswers[q.id] = opinionValue(q).join(',');
    if (q.questionType === 'PASSAGE_MATCH' && !nextAnswers[q.id]) nextAnswers[q.id] = passageMatchValue(q).join(',');
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
          ...
          <div className="space-y-4">
            ...
            {data.questions.map((question, questionIndex) => (
              <div key={questionIndex} className={`grid min-w-0 gap-2 rounded-md border p-3 md:grid-cols-[minmax(0,1fr)_130px] md:items-center ${subAnswerClass(q, questionIndex)}`}>
                <span>{question}</span>
                <select className="input" value={values[questionIndex]} disabled={questionLocked(q)} onChange={(e) => setPassageMatchValue(q, questionIndex, e.target.value)}>
                  <option value="">--</option>
  ...
  function submit() {
    ...
    exam.questions?.forEach((q) => {
      ...
      if (q.questionType === 'MATCHING_DROPDOWN' && !submittedAnswers[q.id]) submittedAnswers[q.id] = q.optionF === 'GRAMMAR_TERM_MATCH' ? grammarTermMatchValue(q).join('|||') : matchingValue(q).join(',');
      if (q.optionF === 'GRAMMAR_SENTENCE_DROPDOWN' && !submittedAnswers[q.id]) submittedAnswers[q.id] = grammarSentenceDropdownValue(q).join('|||');
      if (q.questionType === 'OPINION_MATCH' && !submittedAnswers[q.id]) submittedAnswers[q.id] = opinionValue(q).join(',');
      if (q.questionType === 'PASSAGE_MATCH' && !submittedAnswers[q.id]) submittedAnswers[q.id] = passageMatchValue(q).join(',');
    });
  ...
  return (
    <div className="space-y-4 pb-32 md:pb-24">
      <Link to={`/practice/${skill}`} className="btn btn-muted w-fit"><ArrowLeft size={16} />Quay lại</Link>

      <section className="panel">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="page-title text-2xl font-black leading-tight sm:text-3xl">{exam.title}</h1>
            {!submitted && (
              <p className="mt-2 text-sm font-semibold text-slate-600">
                Đã làm <b className="text-slate-950">{result.answered}</b>/<b>{exam.questions?.length || 0}</b> câu
                {result.total > 0 && <> - Có <b className="text-slate-950">{result.total}</b> ý chấm tự động</>}
              </p>
            )}
            <p className="text-slate-500">{exam.type} - {examDurationMinutes(exam)} phút - {exam.questions?.length || 0} câu hỏi</p>
          </div>
          <div className="flex flex-col gap-2 sm:items-end">
            <div className={`rounded-md border px-4 py-2 text-lg font-black ${timeLeftSeconds <= 60 && !submitted ? 'border-red-200 bg-red-50 text-red-700' : 'border-blue-100 bg-blue-50 text-blue-700'}`}>
              Thời gian: {formatTime(timeLeftSeconds)}
            </div>
            {submitted && (
              <button type="button" className="btn btn-muted w-fit" onClick={resetPractice}>
                <RotateCcw size={16} />Làm lại đề
              </button>
            )}
          </div>
        </div>
        {exam.audioUrl && <audio className="mt-3 w-full" controls src={exam.audioUrl} />}
      </section>
      ...
      {exam.type !== 'WRITING' && standardQuestions().length > 0 && <div className="space-y-3">
        {standardQuestions().map((q, index) => {
          if (index !== currentQuestionIndex) return null;
          const chosen = answers[q.id];
          const reviewed = isQuestionReviewed(q);
          const isCorrect = reviewed && isAnswerCorrect(q);
          const isWrong = reviewed && !isCorrect;
          return (
            <section key={q.id} className="space-y-3" ref={(node) => { questionRefs.current[index] = node; }}>
              {renderReadingQuestionSwitcher(q, index)}
              ...
              <div className={`${hasOwnQuestionHeading(q) ? 'mb-3' : 'mb-5'} flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between`}>
                {!hasOwnQuestionHeading(q) && <h2 className="min-w-0 font-black">{index + 1}. {questionHeading(q)}</h2>}
                {reviewed && (isCorrect ? <CheckCircle2 className="text-green-600" /> : <XCircle className="text-red-600" />)}
              </div>
              {q.audioUrl && !hasOwnAudioLayout(q) && <audio className="w-full" controls src={q.audioUrl} />}
              {isSpeakingQ1ListQuestion(q) ? (
                ...
                renderMixedWritingQuestion(q, index)
              ) : isSpeakingPrompt(q) ? (
                ...
                <div className="space-y-3 rounded-lg bg-slate-50 p-4">
                  {opinionStatements(q).map((statement, statementIndex) => (
                    <div key={statementIndex} className={`grid min-w-0 gap-2 rounded-md border p-3 md:grid-cols-[minmax(0,1fr)_220px] md:items-center ${subAnswerClass(q, statementIndex)}`}>
                      <div><b>{statementIndex + 1}.</b> {statement}</div>
                      <select className="input" value={opinionValue(q)[statementIndex]} disabled={questionLocked(q)} onChange={(e) => setOpinionValue(q, statementIndex, e.target.value)}>
                        <option value="">-- Chọn đáp án --</option>
                        <option value="MAN">Man</option>
                        <option value="WOMAN">Woman</option>
                        <option value="BOTH">Both</option>
                ...
                <div className="space-y-3 rounded-lg bg-slate-50 p-4">
                  {[0, 1, 2, 3].map((personIndex) => {
                    const selected = matchingValue(q)[personIndex];
                    return (
                      <div key={personIndex} className={`grid min-w-0 gap-2 rounded-md border p-3 md:grid-cols-[100px_minmax(0,1fr)] md:items-center ${subAnswerClass(q, personIndex)}`}>
                        <div className="font-semibold">Person {personIndex + 1}</div>
                        <select className="input" value={selected} disabled={questionLocked(q)} onChange={(e) => setMatchingValue(q, personIndex, e.target.value)}>
                          <option value="">-- Chọn đáp án --</option>
                          {dropdownOptions(q).map((option) => (
                            <option key={option.key} value={option.key}>{option.text}</option>
                          ))}
                ...
                <div className="space-y-3">
                  ...
                  if (!text) return null;
                  return (
                    <label key={letter} className="flex min-w-0 cursor-pointer items-start gap-3 text-base">
                      <input type="radio" checked={chosen === letter} disabled={questionLocked(q)} onChange={() => setAnswers({ ...answers, [q.id]: letter })} />
              ...
              {reviewed && (
                <div className={`rounded-md p-3 text-sm ${isWrong ? 'bg-red-50 text-red-800' : 'bg-green-50 text-green-800'}`}>
                  Đáp án đúng: <b>{q.correctAnswer || 'Chưa có'}</b>{q.questionType === 'PARAGRAPH_ORDER' && chosen ? ` - Thứ tự của bạn: ${chosen}` : ''}{q.questionType === 'MATCHING_DROPDOWN' && chosen ? ` - Đáp án của bạn: ${chosen}` : ''}{q.questionType === 'OPINION_MATCH' && chosen ? ` - Đáp án của bạn: ${chosen}` : ''}{q.questionType === 'INLINE_DROPDOWN' && chosen ? ` - Đáp án của bạn: ${chosen}` : ''}{q.questionType === 'PASSAGE_MATCH' && chosen ? ` - Đáp án của bạn: ${chosen}` : ''}{q.explanation ? ` - ${q.explanation}` : ''}
                </div>
              )}
              {!submitted && (
                <div className="question-action-bar md:left-[260px]">
                  <div className="question-action-grid question-action-grid--five">
                    <button className="btn btn-muted question-action-btn" disabled={index === 0} onClick={() => goToPreviousQuestion(index)}>Quay lại</button>
                    ...
                    <button className="btn btn-muted question-action-btn" disabled={index >= standardQuestions().length - 1} onClick={() => goToNextQuestion(index)}>Next question</button>
                    <button type="button" className="btn btn-primary question-action-btn" onClick={submit}><CheckCircle2 size={16} />Chấm điểm</button>
                  </div>
                </div>
              )}
      ...
      {false && !submitted && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-2xl backdrop-blur md:left-[260px]">
          <div className="mx-auto flex max-w-5xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              ...
              <button type="button" className="btn btn-primary w-full sm:w-auto" onClick={submit}>
                <CheckCircle2 size={16} />Nộp bài & chấm điểm
              </button>
            </div>
          </div>
        </div>
